import numpy as np

__all__ = ['ASCAD_MLP_BASELINE', 'ASCAD_CNN_BASELINE', 'NAS_MODEL', 'HYPERBAND_TUNER', 'RANDOM_TUNER', 'BAYESIAN_TUNER',
           'GREEDY_TUNER', 'TUNER_TYPES', 'GUESSING_ENTROPIES', 'QTE_NUM_TRACES', 'SCORES', 'GUESSING_ENTROPY_FINAL', 'ACCURACY', 'NAS_TRIALS_DIRECTORY', "ASCAD", 'ASCAD_DESYNC0',
           'ASCAD_DESYNC50', 'ASCAD_DESYNC0_VARIABLE', 'ASCAD_DESYNC50_VARIABLE', 'ASCAD_DESYNC100_VARIABLE',
           'ASCAD_DESYNC100', 'TRAINED_MODELS', "RESULTS", 'TRAINABLE_PARAMS', 'ASCAD_AES_Sbox',
           'NON_TRAINABLE_PARAMS', 'TOTAL_PARAMS', 'N_DENSE_LAYERS', 'N_CONV_LAYERS', 'ID']


#
# Models
ASCAD_MLP_BASELINE = 'ascad_mlp_baseline'
ASCAD_CNN_BASELINE = 'ascad_cnn_baseline'
NAS_MODEL = 'nas_model'
ID = 'ID'
# Datasets
ASCAD = 'ASCAD'
ASCAD_DESYNC0 = 'ASCAD_desync0'
ASCAD_DESYNC50 = 'ASCAD_desync50'
ASCAD_DESYNC100 = 'ASCAD_desync100'
ASCAD_DESYNC0_VARIABLE = 'ASCAD_desync0_variable'
ASCAD_DESYNC50_VARIABLE = 'ASCAD_desync50_variable'
ASCAD_DESYNC100_VARIABLE = 'ASCAD_desync100_variable'

ASCAD_DATASETS = [ASCAD_DESYNC0, ASCAD_DESYNC50, ASCAD_DESYNC100, ASCAD_DESYNC0_VARIABLE, ASCAD_DESYNC50_VARIABLE,
                  ASCAD_DESYNC100_VARIABLE]
# Important paths
# TRAINED_MODELS = 'trained_models'
# DATASETS = 'datasets'

# Tuner types
HYPERBAND_TUNER = 'hyperband'
RANDOM_TUNER = 'random'
BAYESIAN_TUNER = 'bayesian'
GREEDY_TUNER = 'greedy'
TUNER_TYPES = [HYPERBAND_TUNER, GREEDY_TUNER, RANDOM_TUNER, BAYESIAN_TUNER]

# Attack results file names
SCORES = 'scores'
GUESSING_ENTROPY_FINAL = 'guessing_entropy_final'
GUESSING_ENTROPIES = 'guessing_entropies'
QTE_NUM_TRACES = 'qte_num_traces'
ACCURACY = 'accuracy'
TRAINABLE_PARAMS = 'trainable_params'
NON_TRAINABLE_PARAMS = 'non_trainable_params'
TOTAL_PARAMS = 'total_params'
N_DENSE_LAYERS = "N_DENSE"
N_CONV_LAYERS = "N_CONV"

# Trained models folder names
TRAINED_MODELS = "trained_models"
NAS_TRIALS_DIRECTORY = 'nas_trials_directory'
RESULTS = "results"


# Datasets Sbox
ASCAD_AES_Sbox = np.array([
    
])

