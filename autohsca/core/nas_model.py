...
from autohsca.callbacks import PrintTrialModelCallback
from autohsca.constants import *
from autohsca.utils import get_trained_models_path


class NASModel(BaseEstimator, ClassifierMixin):
    def __init__(self, model_name, num_classes, input_dim, dataset, model_directory, max_trials=100, objective='val_accuracy',
                 overwrite=False, metrics=['accuracy'], tuner='greedy', seed=1234, max_model_size=None, **auto_model_kwargs):
        ...
        dataset_dir = f"{self.dataset.lower()}_{'accuracy'}"
        self.one_dim = True
        self.directory = os.path.join(NAS_TRIALS_DIRECTORY, f"{model_directory}_{self.tuner}")
        self.project_name = dataset_dir
        self.best_model_file_path = os.path.join(get_trained_models_path(), f'{model_name}.tf')
